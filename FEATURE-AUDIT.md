...
| 子分区 | 条目 | 状态 | 说明 |
|---|---|---|---|
| 本体构建 ontology | 15 | ✅ | 对象类型/属性（增删/敏感/必填/枚举/正则约束）/链接类型（外键模型）/动作（CRUD+自定义，执行时校验约束）/状态 |
...
| 检查点 checkpoints / 全局分支 | 11 | 🟡 | 简化版 ✅：对象类型检查点（打点/恢复/差异）+ 命名分支（创建/查看/应用/删除）；完整时间旅行 UI 未做 |
...
| 资源管理器 compass | 10 | 🟡 | 全资产树 ✅（类型/管道/应用/模型/端点/监控/媒体，Compass 式分类浏览）；空间/权限管理 ⬜ |
| FoundryTS | 1 | ⬜ | 未做 |
