...
def list_object_types_for_user(username: str):
    """按 ABAC 过滤：admin 见全部；其余只见有 read 权限的对象类型。"""
    conn = db.get_metadata_conn()
    row = conn.execute("SELECT role FROM users WHERE username=?", (username,)).fetchone()
...
def create_object_type(ot: dict):
    ...
    conn.execute(
        """INSERT OR REPLACE INTO object_types
           (id, name, description, backing_table, primary_key, properties)
           VALUES (?,?,?,?,?,?)""",
        (
            ot["id"],
            ot["name"],
...
def create_action(a: dict):
    ...
    conn.execute(
        """INSERT OR REPLACE INTO actions
           (id, name, description, object_type, operation, parameters)
           VALUES (?,?,?,?,?,?)""",
        (
            a["id"],
            a["name"],
...
def create_pipeline(p: dict):
    ...
    conn.execute(
        ...
        (p["id"], p.get("name", p["id"]), p.get("description", ""), steps),
    )
    conn.commit()
    conn.close()
...
def add_property(type_id: str, prop: dict):
    ...
    try:
        ...
        dconn.close()
    props.append({"key": key, "type": prop.get("type", "string"), "title": title, "column": key,
